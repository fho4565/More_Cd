        ...
        CommandRegister.dispatcher.register(
                Commands.literal("game").requires(s -> s.hasPermission(2))
                        ...
                        .then(Commands.literal("freeMemory").executes(context -> {
                            ...
                            Utils.sendTCdFeedback(context, "mcd.com.fho4565.command.world.freeMemory", String.valueOf(mem));
                            return (int) mem;
                        }))
                        .then(Commands.literal("totalMemory").executes(context -> {
                            long mem = Runtime.getRuntime().totalMemory();
                            Utils.sendTCdFeedback(context, "mcd.com.fho4565.command.world.totalMemory", String.valueOf(mem));
                            return (int) mem;
                        }))
                        .then(Commands.literal("maxMemory").executes(context -> {
                            long mem = Runtime.getRuntime().maxMemory();
                            Utils.sendTCdFeedback(context, "mcd.com.fho4565.command.world.maxMemory", String.valueOf(mem));
                            return (int) mem;
                        }))
                        .then(Commands.literal("gc").executes(context -> {
                            System.gc();
                            Utils.sendTCdFeedback(context, "mcd.com.fho4565.command.world.gc.success");
                            return 1;
                        }))
        );
    }
